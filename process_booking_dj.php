...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $djID = $_POST['dj_id'];  // Decoration ID
    ...
    try {
      ...
      // Insert into tbl_event (added additionaldetails)
        $eventQuery = "INSERT INTO tbl_event (EventType, VendorID, BookingID, CustomerID, DJDetailID, EventStartDate, EventEndDate, EventStatus, EventAmount, EventLocation, AreaID, AdditionalDetails) 
               VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?)";
...
// Bind parameters, including additionaldetails (even if it is NULL)
        mysqli_stmt_bind_param($stmt, 'siiiissdsis', $eventType, $vendorID, $bookingID, $customerID, $djID, $startDate, $endDate, $bookingPrice, $address, $areaID, $additionaldetails);
