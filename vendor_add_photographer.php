...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Get the form data
    $photographyID = $_POST['photographyID'];
    ...
    if ($uploadOk === 0) {
        ...
        if (move_uploaded_file($_FILES["displayImg"]["tmp_name"], $target_file)) {
            ...
            $sql = "INSERT INTO tbl_Photographer_Details (VendorID, PhotographyID, PackageName, PackageDetails, Price, BookingPrice, DisplayImg)
                    VALUES (?, ?, ?, ?, ?, ?, ?)";
            ...
            $stmt->bind_param("iissidd", $vendorID, $photographyID, $packageName, $packageDetails, $price, $bookingPrice, $file_name);

            // Execute the statement
            if ($stmt->execute()) {
                echo "New photographer detail inserted successfully!";
            } else {
...
<body>
    <div class="container">
        <h1>Insert Photographer Details</h1>
        <form method="POST" enctype="multipart/form-data">

            <label for="photographyID">Photography Type:</label>
            <select id="photographyID" name="photographyID" required>
                <option value="">Select Photography Type</option>
                <?php
                require 'config.php'; // Include your database connection
                $sql = "SELECT ID, PhotographyType FROM tbl_Photography";
                $result = $conn->query($sql);

                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        echo "<option value='" . $row['ID'] . "'>" . htmlspecialchars($row['PhotographyType']) . "</option>";
                    }
                } else {
                    echo "<option value=''>No Photography Types Available</option>";
                }
                ?>
            </select>
            ...
            <input type="submit" value="Insert Photographer Detail">
        </form>
    </div>
</body>
</html>
