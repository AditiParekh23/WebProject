...
<html lang="en">
    ...
    <body>
        <div class="container">
            <h2><strong>Caterer Details</strong></h2>
            <div class="row">
                <div class="col-md-8">
                    <?php
                    session_start();
                    ...
                    $VendorID = isset($_GET['vendor_id']) ? intval($_GET['vendor_id']) : '';
                    $CatererID = isset($_GET['caterer_detail_id']) ? intval($_GET['caterer_detail_id']) : '';

                    // Check if VendorID and DJID are set and are numeric
                    if (!empty($VendorID) && is_numeric($VendorID) && !empty($CatererID) && is_numeric($CatererID)) {
                        ...
                        $sql = "
                                ...
                                WHERE 
                                    cd.VendorID = $VendorID AND 
                                    cd.CatererID = $CatererID"; // Added condition for CatererID

                        $result = mysqli_query($conn, $sql);

                        if ($result) {
                            if ($result && mysqli_num_rows($result) > 0) {
                                while ($cater = mysqli_fetch_assoc($result)) {
                                    echo "<div class='details-container'>";

                                    // Display DJ image
                                    echo "<div class='image-container'>";
                                    if (!empty($cater['DisplayImg'])) {
                                        echo "<img src='uploads/vendor_images/" . htmlspecialchars($cater['DisplayImg']) . "' alt='DJ Image'>";
                                    } else {
                                    ...
                                    echo "<p><strong>Name:</strong> " . htmlspecialchars($cater['Name']) . "</p>";
                                    echo "<p><strong>Company Name:</strong> " . htmlspecialchars($cater['CompanyName']) . "</p>";
                                    echo "<p><strong>Type:</strong> " . htmlspecialchars($cater['Type']) . "</p>";
                                    echo "<p><strong>Menu Name:</strong> " . htmlspecialchars($cater['MenuName']) . "</p>";
                                    echo "<p><strong>Price:</strong> ₹" . htmlspecialchars($cater['Price']) . "</p>";
                                    echo "<p><strong>Booking Price:</strong> ₹" . htmlspecialchars($cater['BookingPrice']) . "</p>";
                                    echo "</div>";
                                    echo "</div>"; // Close details
                                    echo "</div>"; // Close details-container
                                    echo "<hr>";

                                    $bookingPrice = $cater['BookingPrice'] ?? 0;
                                }
                ...
                <div class="col-md-4 booking-form">
                    <h3>Booking Form</h3>
                    <form id="bookingForm" action="process_booking_caterer.php" method="POST">
                        ...
                        <input type="hidden" name="caterer_id" value="<?php echo $CatererID ?>">
        ...
        <script>
            ...
            function payNow(event, amount) {
                ...
                var options = {
                    ...
                    currency: "INR",
                    name: "Eventify",
                    description: "Booking for caterer",
