...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($uploadOk === 0) {
        ...
        if (move_uploaded_file($_FILES["displayImg"]["tmp_name"], $target_file)) {
            ...
            $sql = "INSERT INTO tbl_Caterer_Details (VendorID, CatererID, MenuName, MenuDetails, Price, BookingPrice, DisplayImg)
                    VALUES (?, ?, ?, ?, ?, ?, ?)";
            ...
            $stmt->bind_param("iissdds", $vendorID, $catererID, $menuName, $menuDetails, $price, $bookingPrice, $file_name);

            // Execute the statement
            if ($stmt->execute()) {
                echo "New caterer detail inserted successfully!";
            } else {
...
<head>
    ...
    <title>Insert Caterer Details</title>
    <style>
        body {
...
<body>
    <div class="container">
        <h1>Insert Caterer Details</h1>
        <form method="POST" enctype="multipart/form-data">

            <label for="catererID">Caterer Type:</label>
            <select id="catererID" name="catererID" required>
                <option value="">Select Menu Type</option>
                <?php
                require 'config.php'; // Include your database connection
                $sql = "SELECT ID, Type FROM tbl_Caterer";
                ...
                if ($result->num_rows > 0) {
                    ...
                    echo "<option value=''>No Caterer Types Available</option>";
                }
                ?>
            </select>
            ...
            <textarea id="menuDetails" name="menuDetails" rows="4" required></textarea>
            ...
            <input type="submit" value="Insert Caterer Detail">
        </form>
    </div>
</body>
</html>
