...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($uploadOk === 0) {
        ...
        if (move_uploaded_file($_FILES["displayImg"]["tmp_name"], $target_file)) {
            ...
            $sql = "INSERT INTO tbl_DJ_Details (VendorID, DJID, EquipmentDetails, Price, BookingPrice, DisplayImg)
                    VALUES (?, ?, ?, ?, ?, ?)";
            ...
            $stmt->bind_param("iissds", $vendorID, $djID, $equipmentDetails, $price, $bookingPrice, $file_name);

            // Execute the statement
            if ($stmt->execute()) {
                echo "New DJ detail inserted successfully!";
            } else {
...
<body>
    <div class="container">
        <h1>Insert DJ Details</h1>
        <form method="POST" enctype="multipart/form-data">

            <label for="djID">DJ Type:</label>
            <select id="djID" name="djID" required>
                <option value="">Select DJ Type</option>
                <?php
                require 'config.php'; // Include your database connection
                $sql = "SELECT ID, Type FROM tbl_DJ";
                ...
                if ($result->num_rows > 0) {
                    ...
                    echo "<option value=''>No DJ Types Available</option>";
                }
                ?>
            </select>
            ...
            <input type="submit" value="Insert DJ Detail">
        </form>
    </div>
</body>
</html>
