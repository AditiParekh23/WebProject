    ...
    if (!empty($otp) && preg_match("/[0-9]{6}/", $otp)) {
        if ($otp == $_SESSION['sesOtp']) {
            ...
            if ($stmt->execute()) {
                ...
                $sql_customer = "INSERT INTO tbl_customer (Name, Contact_Number, UserID, Address, AreaID) VALUES (?, ?, ?, ?, ?)";
                $stmt = $conn->prepare($sql_customer);
                $stmt->bind_param("ssiss", $name, $contact, $userid, $address, $area);
                ...
                if ($stmt->execute()) {
                    ...
                    echo "<script>alert('Registration successful!'); window.location.replace('customer_dashboard.php');</script>";
                } else {
