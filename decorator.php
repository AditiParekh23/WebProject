...
$sql = "
    SELECT 
        ...
        d.Price, 
        v.Overall_Ratings,
        v.ID AS VendorID,  -- Fetch Vendor ID
        d.ID AS DecorationDetailID -- Fetch DecorationDetail ID for the Book Now button
    FROM 
        tbl_Decoration_Details d
    JOIN 
        tbl_Vendor v ON d.VendorID = v.ID
    JOIN 
...
$DecorationType = [];
$DecorationTypeQuery = "SELECT ID,DecorationType FROM tbl_decoration"; // Assuming you have a venue type table
$DecorationTypeResult = $conn->query($DecorationTypeQuery);
if ($DecorationTypeResult->num_rows > 0) {
    while ($row = $DecorationTypeResult->fetch_assoc()) {
        $DecorationType[] = $row;
    }
}
                    ...
                    <div class="col-md-3 sidebar-wrapper">
                        <div class="sidebar">
                            ...
                            <div id="visible-venue-types">
                                <?php if (!empty($DecorationType)): ?>
                                    <?php foreach (array_slice($DecorationType, 0, 5) as $type): ?>
                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input" id="DecorationType-<?php echo htmlspecialchars($type['ID']); ?>" name="filterDecorationType[]" value="<?php echo htmlspecialchars($type['ID']); ?>">
                                            <label class="form-check-label" for="DecorationType-<?php echo htmlspecialchars($type['ID']); ?>"><?php echo htmlspecialchars($type['DecorationType']); ?></label>
                                        </div>
                                    <?php endforeach; ?>
                                <?php else: ?>
                            ...
                            <div id="hidden-venue-types" style="display: none;">
                                <?php foreach (array_slice($DecorationType, 5) as $type): ?>
                                    <div class="form-check">
                                        <input type="checkbox" class="form-check-input" id="DecorationType-<?php echo htmlspecialchars($type['ID']); ?>" name="filterDecorationType[]" value="<?php echo htmlspecialchars($type['ID']); ?>">
                                        <label class="form-check-label" for="DecorationType-<?php echo htmlspecialchars($type['ID']); ?>"><?php echo htmlspecialchars($type['DecorationType']); ?></label>
                                    </div>
                    ...
                    <div class="col-md-9 content-area">
                        <div class="row">
                            <div class="col-md-12 search-box">
                                <input type="text" id="search" placeholder="Search for Decorators..." aria-label="Search for caterers">
                        ...
                        <div class="row">
                            <?php if (!empty($decorationData)): ?>
                                <?php foreach ($decorationData as $item): ?>
                                    <div class="col-md-4"> <!-- Bootstrap grid column -->
                                        <div class="card">
                                            ...
                                            <div class="card-body">
                                                ...
                                                <!-- Book Now Button with VendorID and DecorationDetailID -->
                                                <a href="decorator_book.php?vendor_id=<?php echo $item['VendorID']; ?>&decoration_detail_id=<?php echo $item['DecorationDetailID']; ?>" class="btn btn-primary">
                                                    Book Now
                                                </a>
                                            </div>
                                        </div>
