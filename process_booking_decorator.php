...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $decorID = $_POST['decor_id'];  // Decoration ID
    ...
    try {
        ...
        // Insert into tbl_event (removed explicit NULL and included additionaldetails)
        $eventQuery = "INSERT INTO tbl_event (EventType, VendorID, BookingID, CustomerID, DecorationDetailID, EventStartDate, EventEndDate, EventStatus, EventAmount, EventLocation, AreaID, AdditionalDetails) 
               VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?)";
...
// Bind parameters, including additionaldetails (even if it is NULL)
        mysqli_stmt_bind_param($stmt, 'siiiissdsis', $eventType, $vendorID, $bookingID, $customerID, $decorID, $startDate, $endDate, $bookingPrice, $address, $areaID, $additionaldetails);
