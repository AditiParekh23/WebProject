...
<html lang="en">
    ...
    <body>
        <div class="container">
            <h2><strong>DJ Details</strong></h2>
            <div class="row">
                <div class="col-md-8">
                    <?php
                    session_start();
                    ...
                    $DJID = $_GET['dj_detail_id'] ?? '';

                    // Check if VendorID and DJID are set and are numeric
                    if (!empty($VendorID) && is_numeric($VendorID) && !empty($DJID) && is_numeric($DJID)) {
                        // SQL query to fetch venue details including the DisplayImg field
                        $sql = "SELECT v.Name, v.Contact_Number, v.CompanyName, d.Type, dd.EquipmentDetails, dd.Price, dd.BookingPrice, dd.DisplayImg 
                        FROM tbl_DJ_Details dd 
                        JOIN tbl_vendor v ON dd.VendorID = v.ID 
                        JOIN tbl_DJ d ON dd.DJID = d.ID 
                        WHERE dd.VendorID = $VendorID AND dd.DJID = $DJID"; // Filter by VendorID and DJID
                        $result = mysqli_query($conn, $sql);

                        if ($result) {
                            if (mysqli_num_rows($result) > 0) {
                                while ($dj = mysqli_fetch_assoc($result)) {
                                    echo "<div class='details-container'>";

                                    // Display DJ image
                                    echo "<div class='image-container'>";
                                    if (!empty($dj['DisplayImg'])) {
                                        echo "<img src='uploads/vendor_images/" . htmlspecialchars($dj['DisplayImg']) . "' alt='DJ Image'>";
                                    } else {
                                    ...
                                    echo "<p><strong>Name:</strong> " . htmlspecialchars($dj['Name']) . "</p>";
                                    echo "<p><strong>Company Name:</strong> " . htmlspecialchars($dj['CompanyName']) . "</p>";
                                    echo "<p><strong>DJ Type:</strong> " . htmlspecialchars($dj['Type']) . "</p>";
                                    echo "<p><strong>Equipment Details:</strong> " . htmlspecialchars($dj['EquipmentDetails']) . "</p>";
                                    echo "<p><strong>Price:</strong> ₹" . htmlspecialchars($dj['Price']) . "</p>";
                                    echo "<p><strong>Booking Price:</strong> ₹" . htmlspecialchars($dj['BookingPrice']) . "</p>";
                                    echo "<p><strong>Contact Number:</strong> " . htmlspecialchars($dj['Contact_Number']) . "</p>";
                                    echo "</div>";
                                    echo "</div>"; // Close details
                                    echo "</div>"; // Close details-container
                                    echo "<hr>";

                                    $bookingPrice = $dj['BookingPrice'] ?? 0;
                                }
                ...
                <div class="col-md-4 booking-form">
                    <h3>Booking Form</h3>
                    <form id="bookingForm" action="process_booking_dj.php" method="POST">
                        ...
                        <input type="hidden" name="dj_id" value="<?php echo $DJID ?>">
