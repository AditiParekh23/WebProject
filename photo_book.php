        ...
        <div class="container">
            ...
            <div class="row">
                <div class="col-md-8">
                    ...
                    $PhotographerDetailID = $_GET['photographer_detail_id'] ?? '';

                    // Check if VendorID and DJID are set and are numeric
                    if (!empty($VendorID) && is_numeric($VendorID) && !empty($PhotographerDetailID) && is_numeric($PhotographerDetailID)) {
                        // SQL query to fetch photographer details including the DisplayImg field
                        $sql = "SELECT v.Name, v.Contact_Number, v.CompanyName, pd.PackageName, pd.PackageDetails, pd.Price, pd.BookingPrice, pd.DisplayImg 
                        FROM tbl_photographer_details pd 
                        JOIN tbl_vendor v ON pd.VendorID = v.ID 
                        WHERE pd.ID = $PhotographerDetailID AND pd.VendorID = $VendorID";
                            ...
                            if (mysqli_num_rows($result) > 0) {
                                while ($photographer = mysqli_fetch_assoc($result)) {
                                   ...
                                   echo "<div class='image-container'>";
                                    if (!empty($photographer['DisplayImg'])) {
                                        echo "<img src='uploads/vendor_images/" . htmlspecialchars($photographer['DisplayImg']) . "' alt='Photographer Image'>";
                                    } else {
                                        echo "<p>No image available for this photographer.</p>";
                                    }
                                    echo "</div>";
                                    ...
                                    echo "<p><strong>Name:</strong> " . htmlspecialchars($photographer['Name']) . "</p>";
                                    echo "<p><strong>Company Name:</strong> " . htmlspecialchars($photographer['CompanyName']) . "</p>";
                                    echo "<p><strong>Package Name:</strong> " . htmlspecialchars($photographer['PackageName']) . "</p>";
                                    echo "<p><strong>Package Details:</strong> " . htmlspecialchars($photographer['PackageDetails']) . "</p>";
                                    echo "<p><strong>Price:</strong> " . htmlspecialchars($photographer['Price']) . "</p>";
                                    echo "<p><strong>Booking Price:</strong> " . htmlspecialchars($photographer['BookingPrice']) . "</p>";
                                    echo "<p><strong>Contact Number:</strong> " . htmlspecialchars($photographer['Contact_Number']) . "</p>";
                                    echo "</div>";
                                    echo "</div>"; // Close details
                                    echo "</div>"; // Close details-container
                                    echo "<hr>";
                                    $bookingPrice = $photographer['BookingPrice'] ?? 0;
                                }
                ...
                <div class="col-md-4 booking-form">
                    <h3>Booking Form</h3>
                    <form id="bookingForm" action="process_booking_photo.php" method="POST">
                        ...
                        <input type="hidden" name="photo_id" value="<?php echo $PhotographerDetailID; ?>">
