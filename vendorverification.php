...
// Insert $passwordHash into the database instead of the plain $password


            $sql_user = "INSERT INTO tbl_user (EmailID, Password) VALUES (?,?)";
            $stmt = $conn->prepare($sql_user);
            $stmt->bind_param("ss", $email, $hashed_password);

            if ($stmt->execute()) {
                $userid = $conn->insert_id;
                $sql_vendor = "INSERT INTO tbl_vendor (Name, Contact_Number, UserID, ServiceID, AreaID, Address, CompanyName, GST_Number) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                $stmt = $conn->prepare($sql_vendor);
                $stmt->bind_param("ssiiisss", $name, $contact, $userid, $service, $area, $address, $company, $gst);
                ...
                if ($stmt->execute()) {
                    ...
                    unset($_SESSION['area']);
                    echo "<script>alert('Registration successful!'); window.location.replace('vendor_dashboard.php');</script>";
                } else {
