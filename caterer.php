...
$sql = "
    SELECT 
        ...
        c.ID AS CatererDetailID
    FROM 
        tbl_caterer_details c
    JOIN 
        tbl_Vendor v ON c.VendorID = v.ID
    JOIN 
...
$catererTypes = [];
$catererTypesQuery = "SELECT ID,Type FROM tbl_caterer"; // Assuming you have a venue type table
$catererTypesResult = $conn->query($catererTypesQuery);
if ($catererTypesResult->num_rows > 0) {
    while ($row = $catererTypesResult->fetch_assoc()) {
        $catererTypes[] = $row;
    }
}
                    ...
                    <div class="col-md-3 sidebar-wrapper">
                        <div class="sidebar">
                            ...
                            <div id="visible-venue-types">
                                <?php if (!empty($catererTypes)): ?>
                                    <?php foreach (array_slice($catererTypes, 0, 5) as $type): ?>
                                        <div class="form-check">
                                            ...
                                            <label class="form-check-label" for="venueType-<?php echo htmlspecialchars($type['ID']); ?>"><?php echo htmlspecialchars($type['Type']); ?></label>
                                        </div>
                                    <?php endforeach; ?>
                                <?php else: ?>
                            ...
                            <div id="hidden-venue-types" style="display: none;">
                                <?php foreach (array_slice($catererTypes, 5) as $type): ?>
                                    <div class="form-check">
                                        <input type="checkbox" class="form-check-input" id="venueType-<?php echo htmlspecialchars($type['ID']); ?>" name="filterVenueType[]" value="<?php echo htmlspecialchars($type['ID']); ?>">
                                        <label class="form-check-label" for="venueType-<?php echo htmlspecialchars($type['ID']); ?>"><?php echo htmlspecialchars($type['Type']); ?></label>
                                    </div>
                    ...
                    <div class="col-md-9 content-area">
                        <div class="row">
                            <div class="col-md-12 search-box">
                                <input type="text" id="search" placeholder="Search for caterers..." aria-label="Search for caterers">
                        ...
                        <div class="row">
                            <?php if (!empty($catererData)): ?>
                                <?php foreach ($catererData as $item): ?>
                                    <div class="col-md-4"> <!-- Bootstrap grid column -->
                                        <div class="card">
                                            ...
                                            <div class="card-body">
                                                ...
                                                <h4> <strong>₹<?php echo htmlspecialchars($item['Price'])."</strong>/plate";?></h4>
                                                <a href=caterer_book.php?vendor_id=<?php echo $item['VendorID']; ?>&caterer_detail_id=<?php echo $item['CatererDetailID']; ?>" class="btn btn-primary">
                                                    Book Now
                                                </a>                                            
                                            </div>
                                        </div>
