...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $photoID = $_POST['photo_id'];  // Decoration ID
    ...
    try {
        ...
        $eventQuery = "INSERT INTO tbl_event (EventType, VendorID, BookingID, CustomerID, PhotographerDetailID, EventStartDate, EventEndDate, EventStatus, EventAmount, EventLocation, AreaID, AdditionalDetails) 
               VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?)";
...
// Bind parameters, including additionaldetails (even if it is NULL)
        mysqli_stmt_bind_param($stmt, 'siiiissdsis', $eventType, $vendorID, $bookingID, $customerID, $photoID, $startDate, $endDate, $bookingPrice, $address, $areaID, $additionaldetails);
