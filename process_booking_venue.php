...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $venueID = $_POST['venue_id'];  // Decoration ID
    ...
    try {
        ...
        $eventQuery = "INSERT INTO tbl_event (EventType, VendorID, BookingID, CustomerID, VenueDetailID, EventStartDate, EventEndDate, EventStatus, EventAmount, EventLocation, AreaID, AdditionalDetails) 
               VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?)";
...
// Bind parameters, including additionaldetails (even if it is NULL)
        mysqli_stmt_bind_param($stmt, 'siiiissdsis', $eventType, $vendorID, $bookingID, $customerID, $venueID, $startDate, $endDate, $bookingPrice, $address, $areaID, $additionaldetails);
