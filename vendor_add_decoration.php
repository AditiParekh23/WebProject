...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($uploadOk === 0) {
        ...
        if (move_uploaded_file($_FILES["displayImg"]["tmp_name"], $target_file)) {
            ...
            $sql = "INSERT INTO tbl_Decoration_Details (VendorID, DecorationID, Description, Price, BookingPrice, DisplayImg)
                    VALUES (?, ?, ?, ?, ?, ?)";
            ...
            $stmt->bind_param("iissds", $vendorID, $decorationID, $description, $price, $bookingPrice, $file_name);

            // Execute the statement
            if ($stmt->execute()) {
                echo "New decoration detail inserted successfully!";
            } else {
...
<head>
    ...
    <title>Insert Decoration Details</title>
    <style>
        body {
...
<body>
    <div class="container">
        <h1>Insert Decoration Details</h1>
        <form method="POST" enctype="multipart/form-data">

            <label for="decorationID">Decoration Type:</label>
            <select id="decorationID" name="decorationID" required>
                <option value="">Select Decoration Type</option>
                <?php
                require 'config.php'; // Include your database connection
                $sql = "SELECT ID, DecorationType FROM tbl_Decoration";
                $result = $conn->query($sql);

                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        echo "<option value='" . $row['ID'] . "'>" . htmlspecialchars($row['DecorationType']) . "</option>";
                    }
                } else {
                    echo "<option value=''>No Decoration Types Available</option>";
                }
                ?>
            </select>
            ...
            <input type="submit" value="Insert Decoration Detail">
        </form>
    </div>
</body>
</html>
